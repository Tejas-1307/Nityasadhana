    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative flex-1">
          <Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[#547070]" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search Shishyas by name or email..."
            className="h-11 w-full rounded-2xl border border-[rgba(63,148,149,0.16)] bg-white pl-10 pr-4 text-[14px] text-[#193B3B] placeholder:text-[#547070] focus:border-[#3F9495] focus:outline-none focus:ring-2 focus:ring-[#3F9495]/20"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative hidden items-center gap-1.5 rounded-2xl border border-[rgba(63,148,149,0.16)] bg-white px-3 py-2 text-[13px] font-semibold text-[#193B3B] sm:flex">
            <ArrowUpDown className="h-3.5 w-3.5 text-[#547070]" />
            <span>Sort:</span>
            <select
              value={sort}
              onChange={(e) => setSort(e.target.value as SortOption)}
              className="bg-transparent font-medium text-[#193B3B] focus:outline-none"
            >
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-[13px] font-semibold transition-all ${
                isActive
                  ? "bg-[#193B3B] text-white shadow-sm"
                  : "bg-white text-[#547070] hover:bg-[#EAF7F4] hover:text-[#193B3B]"
              }`}
            >
              <span>{tab.label}</span>
              {tab.count !== undefined && (
                <span
                  className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                    isActive ? "bg-white/20 text-white" : "bg-[#EAF7F4] text-[#547070]"
                  }`}
                >
                  {tab.count}
                </span>
      ...
      {isFilterSheetOpen && (
        <div className="animate-in fade-in fixed inset-0 z-50 flex items-end bg-black/40 backdrop-blur-sm sm:hidden">
          <div className="w-full rounded-t-3xl border-t border-[rgba(63,148,149,0.16)] bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between pb-4">
              <h3 className="text-[16px] font-bold text-[#193B3B]">Filter Shishyas</h3>
              <button
                type="button"
                onClick={() => setIsFilterSheetOpen(false)}
                className="text-[13px] font-semibold text-[#A9824D]"
              >
                Done
              </button>
            </div>
      ...
      {sortedShishyas.length === 0 ? (
        <Card className="border-[rgba(63,148,149,0.16)] bg-white p-8 text-center shadow-level1">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#EAF7F4] text-[#547070]">
            <UserX className="h-6 w-6" />
          </div>
          <h3 className="mt-3 text-[16px] font-bold text-[#193B3B]">No Shishyas Found</h3>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-[12px] text-[#547070]">
                        ...
                        {primarySignal && (
                          <span className="font-semibold text-[#A9824D]">
                            · {primarySignal.reason}
                          </span>
                        )}

                        {item.additionalSignalsCount > 0 && (
                          <span className="font-semibold text-[#547070]">
